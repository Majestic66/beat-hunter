...
const Sidebar: React.FC<SidebarProps> = ({ gameState, onSelectArtist }) => {
  return (
    <div className="w-80 h-full glass border-r border-white/10 p-4 md:p-6 flex flex-col gap-4 md:gap-6 overflow-y-auto">
      <div className="flex items-center gap-3 mb-2 md:mb-4">
        <div className="bg-violet-600 p-2 rounded-lg neon-border">
          <Music className="text-white w-5 h-5 md:w-6 md:h-6" />
        </div>
        <div className="flex flex-col min-w-0">
          <h1 className="text-lg md:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-violet-400 to-fuchsia-400 tracking-tighter">
            BEAT HUNTER PRO
          </h1>
          ...
          {gameState.signedArtists.length === 0 ? (
            ...
            gameState.signedArtists.map(artist => (
              <div 
                ...
                className="group relative flex items-center gap-3 p-2.5 md:p-2 rounded-lg hover:bg-violet-600/20 active:bg-violet-600/30 transition-all cursor-pointer border border-transparent hover:border-violet-500/30 touch-manipulation min-h-[60px]"
              >
                <img src={artist.avatar} alt={artist.name} className="w-12 h-12 md:w-10 md:h-10 aspect-square rounded-md object-cover grayscale group-hover:grayscale-0 transition-all flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate text-slate-200">{artist.name}</p>
                  <p className="text-[10px] text-slate-400 uppercase truncate">{artist.genre} • {artist.potential}% Pot.</p>
                </div>
                <Play className="w-3 h-3 text-violet-400 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
              </div>
            ))
          )}
        </div>
      ...
      <div className="mt-auto pt-4 md:pt-6 border-t border-white/10">
        <div className="flex items-center justify-between text-[10px] text-slate-500 uppercase font-bold tracking-widest">
          ...
          <span className="text-violet-400 cursor-pointer hover:underline touch-manipulation">Configuration</span>
        </div>
      </div>
    </div>
  );
