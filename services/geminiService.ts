...
import { japanArtists } from "../utils/randomArtistSelectorJapan";
...
import { colombiaArtists } from "../utils/randomArtistSelectorColombia";
...
import { indiaArtists } from "../utils/randomArtistSelectorIndia";
...
function generateArtistName(countryName: string, seed: number): string {
  ...
  if (countryName === 'Japan') {
    // Utiliser la liste des artistes japonais fournie
    const index = seed % japanArtists.length;
    return japanArtists[index].split(' → ')[0]; // Prendre seulement le nom, pas le style
  }
  if (countryName === 'Brazil') {
    // Utiliser la liste des artistes brésiliens fournie
    const index = seed % brazilArtists.length;
    return brazilArtists[index].split(' – ')[0]; // Prendre seulement le nom, pas le style
  }
  if (countryName === 'Nigeria') {
    // Utiliser la liste des artistes nigérians fournie
    const index = seed % nigeriaArtists.length;
    return nigeriaArtists[index].split(' – ')[0]; // Prendre seulement le nom, pas le style
  }
  ...
  if (countryName === 'Colombia') {
    // Utiliser la liste des artistes colombiens fournie
    const index = seed % colombiaArtists.length;
    return colombiaArtists[index].split(' – ')[0]; // Prendre seulement le nom, pas le style
  }
  if (countryName === 'Germany') {
    // Utiliser la liste des artistes allemands fournie
    const index = seed % germanyArtists.length;
    return germanyArtists[index].split(' – ')[0]; // Prendre seulement le nom, pas le style
  }
  if (countryName === 'South Africa') {
    // Utiliser la liste des artistes sud-africains fournie
    const index = seed % southAfricaArtists.length;
    return southAfricaArtists[index].split(' – ')[0]; // Prendre seulement le nom, pas le style
  }
  ...
  if (countryName === 'India') {
    // Utiliser la liste des artistes indiens fournie
    const index = seed % indiaArtists.length;
    return indiaArtists[index].split(' → ')[0]; // Prendre seulement le nom, pas le style
  }
  if (countryName === 'Mexico') {
    // Utiliser la liste des artistes mexicains fournie
    const index = seed % mexicoArtists.length;
    return mexicoArtists[index].split(' – ')[0]; // Prendre seulement le nom, pas le style
  }
  if (countryName === 'Italy') {
    // Utiliser la liste des artistes italiens fournie
    const index = seed % italyArtists.length;
    return italyArtists[index].split(' – ')[0]; // Prendre seulement le nom, pas le style
  }
  if (countryName === 'Spain') {
    // Utiliser la liste des artistes espagnols fournie
    const index = seed % spainArtists.length;
    return spainArtists[index].split(' – ')[0]; // Prendre seulement le nom, pas le style
  }
